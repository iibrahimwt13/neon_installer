...
def main():
    ...
    if Sonuc == "2":
        ...
        if soup.title.string == "Create new application":
            ...
            app_title = soru("APP name nə olsun? (Oto qurulma üçün boş buraxın): ")
            if app_title == '':
                app_title = choice(["n", "ne", "neo", "neon", "telethon", "pyrogram"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
            
            app_shortname = soru("APP-name qısa nə olsun? (Oto qurulma üçün boş buraxın): \[5-32 simvol\]: ")
            if app_shortname == '':
                app_shortname = choice(["n", "ne", "neon", "o2", "non", "oksi"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
            ...
            if app == "ERROR":
                hata("(!) Telegram kiliddən çıxarılmağınız bloklandı. Xahiş edirəm string-i yenidən başladın (restart verin).")
                exit(1)
        ...
        hata("(!) Bilinməyən seçim (get təzdən gəl).")
        exit(1)
